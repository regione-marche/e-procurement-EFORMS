...
import { cloneDeep, get, has, isArray, isEmpty, remove, set } from 'lodash-es';
...
@Component({
  templateUrl: `new-enotice-section.widget.html`,
  encapsulation: ViewEncapsulation.None
})
export class NewEnoticeSectionWidget extends AbstractFormComponent implements OnInit, OnDestroy {
  @HostBinding('class') classNames = `new-enotice-section`;
  ...
  private _tracciatoJson: IDictionary<any>;
  private _jwtTokenClaims: IDictionary<any>;
  ...
  protected onAfterViewInit(): void {
    if (isEmpty(this.notice)) {
      ...
      if (token != null) {
        this._jwtTokenClaims = CryptoUtils.getJwtTokenClaims(token);
      }
        ...
        this.loadSingleNoticeType()
          .pipe(
            ...
            concatMap(() => {
              ...
              this.loadResultsItems(); // carica i risultati per la visualizzazione sotto forma di tabella
              this.disableAllForms();
  ...
  protected onConfig(config: any): void {
    if (config != null) {
      ...
      if (get(this.config.body, 'sidebar') != null) {
  ...
  public async onButtonClick(button: SdkButtonGroupOutput): Promise<void> {
    if (button != null) {
      ...
      } else if (button.code == 'test-formreactive') {
        // Popola la formreactivecontent con i valori per questa enotice che sono
        // in app-eforms-notice-editor in base al tipo.

        (async () => {
          await (async () => {
            for (let i = 0; i < this._formService.firstLevelItems.length; i++) {
              if (
                !(
                  this._formService.firstLevelItems[i]?.items?.length != null &&
                  this._formService.firstLevelItems[i]?.items?.length == 0
                )
              ) {
                if (this._formService.firstLevelItems[i]?.items?.length > 0) {
                  this.manageFirstLevelItemToggle(this._formService.firstLevelItems[i].items[0]);
                } else {
                  this.manageFirstLevelItemToggle(this._formService.firstLevelItems[i]);
                }
          ...
          let suggestedAndObligatoryFieldsWithValues =
            this._dataService.suggestedAndObligatoryFieldsWithValues[this._dataService.selectedNotice];
          Object.keys(this._formService.dictionaryOfAllSuggestedFormControls).forEach((key) => {
            const value = suggestedAndObligatoryFieldsWithValues
              .find((item) => item.startsWith(key + ' '))
              ?.substring(key.length + 1);

            const formControl = this._formService.dictionaryOfAllSuggestedFormControls[key];

            //per i cpv
            if (value != null) {
              const match = value.match(/key: (\d+) label: ([^"]+) - \d+ children/);
              if (match) {
                const key = match[1];
                const label = match[2];
                ...
                formControl.patchValue(transformedObject);
              } else {
          ...
          // this._formReactiveContent;
          // this.metadataFormGroup
          // this._formService.arrayOfSuggested = Array.from(new Set(this._formService.arrayOfSuggested));

          //devo riempire il campo value con i valori necessari.
          // this._formService.dictionaryOfAllSuggestedFormControls;
          // this._formService.dictionaryOfAllSuggestedTypes;

          // // Use the patched values to process suggested form controls and types
          // Object.keys(this._formService.dictionaryOfAllSuggestedFormControls).forEach((key) => {
          //   const formControl = this._formService.dictionaryOfAllSuggestedFormControls[key];
          //   const type = this._formService.dictionaryOfAllSuggestedTypes[key]?.type;
          //   const combo = this._formService.dictionaryOfAllSuggestedCombobox[key];

          //   switch (type) {
          //     case 'amount':
          //       formControl.patchValue(100);
          //       break;
          //     case 'code':
          //       if (this._formService.dictionaryOfAllSuggestedTypes[key].codeList.value.type == 'hierarchical') {
          //         // formControl.patchValue({
          //         //   'key': '18311000',
          //         //   'label': 'Sottovesti - [18311000]',
          //         //   'children': []
          //         // });
          //         formControl.patchValue({
          //           'key': '65000000',
          //           'label': 'Servizi Pubblici - [65000000]',
          //           'children': []
          //         });
          //       } else {

          //         key
          //         formControl.patchValue(combo);
          ...
          //     case 'date':
          //       formControl.patchValue('2025-02-29Z');
          //       break;

          //     // 'value' : '^[A-Za-z0-9!#$%&amp;''*+/=?_-]+(\\.[A-Za-z0-9!#$%&amp;''*+/=?_-]+)*@([A-Za-z0-9]([A-Za-z0-9_-]*[A-Za-z0-9])?\\.)+([A-Za-z]{2,})$',
          //     case 'email':
          //       formControl.patchValue('test@gmail.com');
          //       break;

          //     //      'value' : '^([a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}-(0[1-9]|[1-9]\\d)|(\\d{1,8})-(19|20)\\d\\d)$',
          //     case 'id':
          //       formControl.patchValue('12345678-1234-4abc-89ab-123456789012-01-2025');
          //       break;
          //     case 'integer':
          //       formControl.patchValue(100);
          //       break;
          //     // case 'measure':
          //     //   formControl.patchValue('1.75m');
          //     //   break;
          //     case 'number':
          ...
          //     // 'value' : '^((\\(\\+?[0-9]+\\))|\\+?[0-9]+)( - |-| )?(((\\(\\d+\\))|\\d+)( - |-| )?)*(\\d+)( )?$',
          //     case 'phone':
          //       formControl.patchValue('+39 123 456 7890');
          //       break;
          //     case 'text':
          //       formControl.patchValue('Test');
          //       break;
          ...
          //     case 'time':
          //       formControl.patchValue('12:00Z');
          //       break;

          //     //'value' : '((^(http|HTTP|https|HTTPS|ftp|FTP|ftps|FTPS|sftp|SFTP)://)|(^(w|W){3}(\\d)?\\.))[\\w\\?!\\./:;,\\-_=#+*%@&quot;\\(\\)&amp;]+',
          //     case 'url':
          //       formControl.patchValue('http://example.com');
          //       break;
          //     case 'indicator':
          //       formControl.patchValue(true);
          //       break;

          //     // // 'value' : '^( BUYER|RESULT|((RES|TEN|TPA|TPO|ORG|UBO)-\\d{4}))$',
          //     case 'id-ref':
        ...
        if (body != null && !executeValidation) {
          this._apiService.xsdValidation(body).subscribe({
            next: (response: ResponseResult<any>) => {
              console.log('response >>>', response);
              outputModalConfig.content = JSON.stringify(response.data, null, 4);
  ...
  private elaborateSuggestedFields = () => {
    ...
    if (get(this.config, 'suggestedAndObligatoryFieldsWithValues') != null) {
      this._dataService.elaborateSuggestedAndObligatoryFieldsWithValues(
        get(this.config, 'suggestedAndObligatoryFieldsWithValues')
      );
    }
  };
  ...
  private elaborateRequiredFields = () => {
    if (get(this.config, 'requiredFields') != null) {
  ...
  private elaborateSingleNoticeType = (response: ResponseResult<IDictionary<any>>) => {
    ...
    //filtro la label 'Rettifica' dal menu laterale (layout-side-menu-tabs.widget), in quanto non deve esserci in caso di nuova notice.
    if (!this.isRettifica)
      data.content = data.content.filter((element) => element.id !== Constants.StandardIdentifiers.RETTIFICA_GROUP);
  ...
  private elaborateFirst = () => {
    let content = this._dataService.selectedNoticeType.content.map((c: any) => {
      ...
      c = this._formService.checkForbiddenFields(this.notice, c);
  ...
  /**
   * Metodo che inizializza la form reactive
   */
  private initForm = () => {
    // Load tracciato da applicativo esterno
    let tracciatoJson: IDictionary<any> = this._sdkSessionStorageService.getItem<IDictionary<any>>(
      Constants.TRACCIATO_SESSION_KEY
    );
    // Salvo il tracciato in locale
    this._tracciatoJson = tracciatoJson;

    Object.keys(this._formContent).forEach((chiave: string) => {
      let selectedFormContent: any = this._formContent[chiave];

      let initialPartial: IDictionary<any> | Array<IDictionary<any>> = this._formService.getInitialExternalJsonPartial(
        tracciatoJson,
        chiave,
        selectedFormContent._repeatable
      );

      let formGroups: Array<FormGroup> = [];

      if (initialPartial && isArray(initialPartial)) {
        initialPartial.forEach((elem: IDictionary<any>) => {
          try {
            let formGroup = this._formService.createFormGroupForSelectedFormContent(
              this.notice,
              selectedFormContent,
              elem
            );
        ...
        try {
          formGroup = this._formService.createFormGroupForSelectedFormContent(
            this.notice,
            selectedFormContent,
            initialPartial
          );
      ...
      if (selectedFormContent.id != 'GR-LotsGroup') {
        if (selectedFormContent._repeatable) {
          this._formReactiveContent[chiave] = {};
          formGroups.forEach((fg: FormGroup) => {
            let id: string;

            if (selectedFormContent._idScheme == 'LOT') {
              id = fg.get('BT-137-Lot').value;
            } else if (selectedFormContent._idScheme == 'GLO') {
              id = fg.get('BT-137-LotsGroup').value;
            } else if (selectedFormContent._idScheme == 'PAR') {
              id = fg.get('GR-Part-Purpose').get('BT-137-Part').value;
            }

            if (isEmpty(id)) {
              id = selectedFormContent._idScheme + '-' + '0001';
            }

            this._formReactiveContent[chiave][id] = cloneDeep(fg);
          });
        } else {
          this._formReactiveContent[chiave] = cloneDeep(formGroups[0]);
        }
      }
      this._formReactiveContentOriginal[chiave] = cloneDeep(formGroups[0]);
    });
  ...
  private fillInfoReactive = (): void => {
    ...
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.UBL_VERSION_FIELD]?.patchValue(
      this._dataService.basicMetaData.fieldsJson.ublVersion
    );

    // Set SDK version in the form.
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.SDK_VERSION_FIELD]?.patchValue(
      this._dataService.basicMetaData.fieldsJson.sdkVersion
    );

    // Set the version id
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.NOTICE_VERSION_FIELD]?.patchValue('01');
    ...
    this.noticeInfo = {
      ...
      subTypeId: this._selectedNoticeSubtype.subTypeId,
      uuid: null,
    ...
    if (this.metadataFormGroup.controls[Constants.StandardIdentifiers.PROCEDURE_IDENTIFIER] != null) {
      this.metadataFormGroup.controls[Constants.StandardIdentifiers.PROCEDURE_IDENTIFIER]?.patchValue(
        CryptoUtils.generateUuid()
      );
    }

    // Notice Subtype
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.NOTICE_SUBTYPE_FIELD]?.patchValue(this.notice);

    // Language
    this.metadataFormGroup.controls['BT-702(a)-notice']?.patchValue(get(I18N.Iso6391ToIso6393Map, this.selectedLang));

    // Notice Type
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.NOTICE_TYPE_FIELD]?.patchValue(
      this._selectedNoticeSubtype.type
    );

    // Form Type
    this.metadataFormGroup.controls[Constants.StandardIdentifiers.FORM_TYPE_FIELD]?.patchValue(
      this._selectedNoticeSubtype.formType
    );
    ...
    //this.metadataFormGroup.controls['OPP-012-notice']?.patchValue(new Date());
    ...
    if (this.isRettifica) {
      ...
      let noticeUUID: string = this._uuidNotice ?? CryptoUtils.generateUuid();
      this.metadataFormGroup.controls[Constants.StandardIdentifiers.NOTICE_UUID_FIELD].patchValue(noticeUUID);
      this.noticeInfo.uuid = noticeUUID;
    }
  };
  ...
  override manageRemoveElementEvent(obj: AccordionOutput): void {
    /*
      ...
      perche' cio' mi impone di deselezionare la form
    */
    ...
    // // Rimuovo il form group dalla mappa
    // if (obj.id == GR_RESULT_ID) {
    //   // sezione risultati
    //   if (has(this._formContent, obj.id) && has(this._formReactiveContent, obj.id)) {
    //     Object.keys(this._formReactiveContent[obj.id].value).forEach((key) => {
    //       if (key.includes(obj.key)) {
    //         // switch per sezioni dei risultati
    //         switch (key) {
    //           // parti offerenti
    //           case 'GR-TenderingParty-Section':
    //             var i = 0;
    //             Object.values(this._formReactiveContent[obj.id].value[key]).forEach((value) => {
    //               let tpaValue = value[0]['OPT-210-Tenderer'];
    //               if (tpaValue.includes(obj.index)) {

    //                 delete this._formReactiveContent[obj.id].value[key]['GR-TenderingParty'][i];
    //                 this._formService.removeIdField('TPA', tpaValue);
    //               }
    //               i++;
    //             });
    //             break;
    ...
    //             var i = 0;
    //             Object.values(this._formReactiveContent[obj.id].value[key]).forEach((value) => {
    //               let conValue = value[0]['GR-SettledContract-1']['OPT-316-Contract'];
    //               if (conValue.includes(obj.index)) {

    //                 delete this._formReactiveContent[obj.id].value[key]['GR-SettledContract'][i];
    //                 this._formService.removeIdField('CON', conValue);
    //               }
    //               i++;
    //             });
    //             break;
    ...
    //             var i = 0;
    //             Object.values(this._formReactiveContent[obj.id].value[key]).forEach((value) => {
    //               let tenValue = value[0]['GR-Tender']['OPT-321-Tender'];
    //               if (tenValue.includes(obj.index)) {

    //                 delete this._formReactiveContent[obj.id].value[key]['GR-LotTender'][i];
    //                 this._formService.removeIdField('TEN', tenValue);
    //               }
    //               i++;
    //             });
    //             break;
    ...
    //             var i = 0;
    //             Object.values(this._formReactiveContent[obj.id].value[key]).forEach((value) => {
    //               let resValue = value[0]['OPT-322-LotResult'];
    //               if (resValue.includes(obj.index)) {

    //                 delete this._formReactiveContent[obj.id].value[key]['GR-LotResult'][i];
    //                 this._formService.removeIdField('RES', resValue);
    //               }
    //               i++;
    //             });
    //             break;
