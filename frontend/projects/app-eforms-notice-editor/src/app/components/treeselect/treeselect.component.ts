...
} from './treeselect.interface';

export const TREESELECT_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => TreeSelectCustom),
  multi: true
};
/**
    ...
    >
      <div class="p-hidden-accessible">
        <input
          ...
          [attr.aria-label]="
            ariaLabel || (label === 'p-emptylabel' ? undefined : label)
          "
          pAutoFocus
          [autofocus]="autofocus"
        />
          ...
          >
            ...
            >
              <div class="p-treeselect-filter-container">
                <input
                  ...
                  [(ngModel)]="filterValue"
                  (ngModelChange)="debounce.next($event)"
                />
                <SearchIcon
                ...
                <span
                  *ngIf="filterIconTemplate"
                  class="p-treeselect-filter-icon"
                >
                  <ng-template
              ...
              <button class="p-treeselect-close p-link" (click)="hide()">
                ...
                <span *ngIf="closeIconTemplate">
                  <ng-template
                    *ngTemplateOutlet="closeIconTemplate"
                  ></ng-template>
                </span>
              </button>
            </div>
            ...
            >
              ...
              >
                ...
                <ng-template
                  pTemplate="togglericon"
                  let-expanded
                  *ngIf="itemTogglerIconTemplate"
                >
                  <ng-container
                    *ngTemplateOutlet="
                      itemTogglerIconTemplate;
                      context: { $implicit: expanded }
                    "
                  ></ng-container>
                </ng-template>
                <ng-template
                ...
                <ng-template
                  ...
                  *ngIf="itemLoadingIconTemplate"
                >
                  <ng-container
                    *ngTemplateOutlet="itemLoadingIconTemplate"
                  ></ng-container>
                </ng-template>
              </p-tree>
            </div>
...
export class TreeSelectCustom implements AfterContentInit, OnDestroy {
  /**
   ...
   * @group Props
   */
  @Input({ transform: booleanAttribute }) propagateSelectionDown: boolean =
    true;
  /**
   ...
   * @group Emits
   */
  @Output() onNodeExpand: EventEmitter<TreeSelectNodeExpandEvent> =
    new EventEmitter<TreeSelectNodeExpandEvent>();
  /**
   * Callback to invoke when a node is collapsed.
   * @param {TreeSelectNodeCollapseEvent} event - Custom node collapse event.
   * @group Emits
   */
  @Output() onNodeCollapse: EventEmitter<TreeSelectNodeCollapseEvent> =
    new EventEmitter<TreeSelectNodeCollapseEvent>();
   ...
   * @group Emits
   */
  @Output() onFilter: EventEmitter<TreeFilterEvent> =
    new EventEmitter<TreeFilterEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onNodeUnselect: EventEmitter<TreeNodeUnSelectEvent> =
    new EventEmitter<TreeNodeUnSelectEvent>();
  /**
   * Callback to invoke when a node is selected.
   * @param {TreeNodeSelectEvent} event - node select event.
   * @group Emits
   */
  @Output() onNodeSelect: EventEmitter<TreeNodeSelectEvent> =
    new EventEmitter<TreeNodeSelectEvent>();
  ...
  @ViewChild('firstHiddenFocusableEl')
  firstHiddenFocusableElementOnOverlay: Nullable<ElementRef>;

  @ViewChild('lastHiddenFocusableEl')
  ...
  ngOnInit() {
    ...
    this.updateTreeState();
    this._debounceSubscription = this.debounce
      .pipe(
        tap(() => {
          if (
            isEmpty(this.filterValue) ||
            (this.filterValue && this.filterValue.length >= 3)
          ) {
            if (!this.loading) {
              this.loading = true;
            }
  ...
  ngOnDestroy(): void {
    if (this._debounceSubscription) this._debounceSubscription.unsubscribe();
  }
  ...
  onOverlayAnimationStart(event: AnimationEvent) {
    switch (event.toState) {
      case 'visible':
        if (this.filter) {
          ObjectUtils.isNotEmpty(this.filterValue) &&
            this.treeViewChild?._filter(<any>this.filterValue);
          this.filterInputAutoFocus &&
  ...
  onFilterInput(event: Event) {
    // Limit minimum character search to 3
    if (
      isEmpty(this.filterValue) ||
      (this.filterValue && this.filterValue.length >= 3)
    ) {
  ...
  onFirstHiddenFocus(event) {
    const focusableEl =
      event.relatedTarget === this.focusInput?.nativeElement
        ? DomHandler.getFirstFocusableElement(
            this.overlayViewChild?.overlayViewChild?.nativeElement,
            ':not([data-p-hidden-focusable="true"])'
          )
        : this.focusInput?.nativeElement;
  ...
  onLastHiddenFocus(event) {
    const focusableEl =
      event.relatedTarget === this.focusInput?.nativeElement
        ? DomHandler.getLastFocusableElement(
            this.overlayViewChild?.overlayViewChild?.nativeElement,
            ':not([data-p-hidden-focusable="true"])'
          )
        : this.focusInput?.nativeElement;
  ...
  findIndexInSelection(node: TreeNode) {
    if (this.value) {
      ...
      return value.findIndex(
        (selectedNode) =>
          selectedNode === node ||
          (selectedNode.key === node.key && selectedNode.key !== undefined)
      );
    }
    return -1;
  }
  ...
  //   return value.length
  //     ? value?.map((node: TreeNode) => node.label).join(', ')
    ...
    return Array.isArray(value)
      ? value.length
        ...
        : this.selectionMode === 'single' &&
          typeof this.value?.label === 'string'
        ? this.value.label
        : this.placeholder
      : this.selectionMode === 'single' && typeof this.value?.label === 'string'
      ? this.value.label
      : this.placeholder;
  }
}
